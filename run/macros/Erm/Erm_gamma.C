
int ngammas;
const int NMAXGMMA = 500;
float pfo_gamma_e[NMAXGMMA], mcr_gamma_e[NMAXGMMA];
bool mcr_gamma_iscreatedinsim[NMAXGMMA];

void DrawSlice(TTree* T = 0, TH1D* h1 = 0, TH1D* h2 = 0, TH1D* h3 = 0, TCanvas* c=0)
{
  h1->Reset();
  h2->Reset();
  int nevents = T->GetEntries();

  for (int ev = 0; ev < nevents; ev++) {
    
    T->GetEntry(ev);
    for ( int ip = 0; ip < ngammas; ip++ ) {
      if(mcr_gamma_e[ip]!=0){
	  	h1->Fill(pfo_gamma_e[ip]/mcr_gamma_e[ip]);
      	if ( !mcr_gamma_iscreatedinsim[ip]) h2->Fill(pfo_gamma_e[ip]/mcr_gamma_e[ip]);
      	else h3->Fill(pfo_gamma_e[ip]/mcr_gamma_e[ip]);
	  }
    }
  }
  c->cd();
  c->SetLogy();
  h1->SetTitle(";E_{RC}/E_{MC} for gamma;");
  h1->Draw();
  h1->SetLineStyle(7);
  h2->SetLineColor(4);
  h2->Draw("same");
  h3->SetLineColor(2);
  h3->Draw("same");
  TLegend *leg = new TLegend(0.15,0.85,0.45,0.7);
  leg->SetTextFont(132);
  leg->SetTextSize(0.03); 
  leg->SetTextAlign(12);
  leg->AddEntry(h1,"All PFOs","f"); 
  leg->AddEntry(h2,"PFOs created in generator","f"); 
  leg->AddEntry(h3,"PFOs created in simulation","f"); 
  leg->Draw();
  stringstream fname;
  fname << "$HOME/work/MonoPhoton/run/hists/MP/Erm/Erm_gamma.pdf" << ends;
  c->Print(fname.str().data());
}

void Erm_gamma ()
{
  TFile* fin = new TFile("$HOME/work/MonoPhoton/run/MP.root");
  TTree* data = static_cast<TTree*>(fin->Get("evtdata"));

  TH1D* hall = new TH1D("hall","",300,0,3);
  TH1D* hgen = new TH1D("hgen","",300,0,3);
  TH1D* hsim = new TH1D("hsim","",300,0,3);
  TCanvas* c = new TCanvas("c","",600,400);

  data->SetBranchAddress("ngammas",&ngammas);
  data->SetBranchAddress("pfo_gamma_e",pfo_gamma_e);
  data->SetBranchAddress("mcr_gamma_e",mcr_gamma_e);
  data->SetBranchAddress("mcr_gamma_iscreatedinsim",mcr_gamma_iscreatedinsim);

  DrawSlice(data,hall,hgen,hsim,c);

}
